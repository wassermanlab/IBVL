...
process fastqc {
	...
	publishDir "$params.outdir_ind/${version}/${assembly}/QC/Fastqc/", mode: 'copy'
...
process sorted_bam_files {
	...
	publishDir "$params.outdir_ind/${version}/${assembly}/BAM/", mode: 'copy'
	...
	val assembly from params.assembly
        ...
        path ref_genome_fai from params.ref_genome_fai
        path ref_genome_amb from params.ref_genome_amb
        path ref_genome_ann from params.ref_genome_ann
        path ref_genome_bwt from params.ref_genome_bwt
        path ref_genome_pac from params.ref_genome_pac
        path ref_genome_sa from params.ref_genome_sa
	...
	bwa mem -t 8 -R '@RG\\tID:${sampleId}\\tSM:${sampleId}' ${ref_genome} ${reads} | samtools view -Sb | samtools sort -o ${sampleId}_sorted.bam
...
process Mosdepth {
	...
	publishDir "$params.outdir_ind/${version}/${assembly}/QC/Mosdepth/", mode: 'copy'
...
process Picard_CollectWgsMetrics {
        cpus 8
	publishDir "$params.outdir_ind/${version}/${assembly}/QC/Picard_CollectWgsMetrics/", mode: 'copy'
...
process Picard_CollectAlignmentSummaryMetrics {
        cpus 8
	publishDir "$params.outdir_ind/${version}/${assembly}/QC/Picard_CollectAlignmentSummaryMetrics/", mode: 'copy'
...
process Picard_QualityScoreDistribution {
        cpus 8
	publishDir "$params.outdir_ind/${version}/${assembly}/QC/Picard_QualityScoreDistribution/", mode: 'copy'
...
process multiqc_indiv {
	...
	publishDir "$params.outdir_ind/${version}/${assembly}/QC/multiqc/", mode: 'copy'
	...
	multiqc ${params.outdir_ind}/${version}/${assembly}/QC/
	"""
}
...
process deepvariant_call {
	...
	publishDir "$params.outdir_ind/${version}/${assembly}/DeepVariant/", mode: 'copy'
...
process gvcfs_txt {
	...
	path outdir_pop from params.outdir_pop
 	...
 	publishDir "$params.outdir_pop/${version}/${assembly}/", mode: 'copy'
	...
	script:
	"""
	find $params.outdir_ind/${version}/${assembly}/DeepVariant/ -name "*.g.vcf.gz" > gvcfs.txt
	"""
}
...
process Extract_MT_Read {
	...
	file (index) from sorted_bam_files_index2
        val Mitochondrial_chromosome from params.Mitochondrial_chromosome
...
process MT_Liftover {
	...
	file '*_rejected_variants.vcf' into rejected_liftover
        ...
        publishDir "$params.outdir_ind/${version}/${assembly}/Mutect2/Liftover/",  pattern: "*_rejected_variants.vcf",  mode: 'copy'
...
process MT_MergeVcfs {
	...
	publishDir "$params.outdir_ind/${version}/${assembly}/Mutect2/", mode: 'copy'
...
process MT_vcfs_txt {
	...
	script:
	"""
	find $params.outdir_ind/${version}/${assembly}/Mutect2/ -name "*_trimmed.vcf.gz" > MT_vcfs.txt
	"""
}
...
process Bcftools_stats {
	...
	publishDir "$params.outdir_pop/${version}/${assembly}/QC/Bcftools_stats/", mode: 'copy'
...
//	publishDir "$params.outdir_pop/${version}/${assembly}/QC/Vcftools_TsTv/", mode: 'copy'
...
process annotation_table {
        ...
        val version from params.version
        path outdir_pop from params.outdir_pop
	path vep_cache from params.vep_cache
        val assembly from params.assembly
        ...
        publishDir "$params.outdir_pop/${version}/${assembly}/", mode: 'copy', pattern : '*tab*'
        publishDir "$params.outdir_pop/${version}/${assembly}/QC/VEP/", mode: 'copy', pattern : '*_VEP_stats*'
...
process multiqc_pop {
        ...
        publishDir "$params.outdir_pop/${version}/${assembly}/QC/multiQC/", mode: 'copy'
        ...
        singularity exec -B /mnt/scratch/SILENT/Act3/ -B /mnt/common/SILENT/Act3/ /mnt/common/SILENT/Act3/singularity/multiqc-1.9.sif \
        multiqc $params.outdir_pop/${version}/${assembly}/QC/
        """
}
